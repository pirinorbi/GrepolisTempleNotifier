    ...
    try {
        ...
        function loadCityIndex(globals) {
            ...
            var translate = {
                ...
                CMD_SHARE_SYNCED: 'Synchronized âœ“',
                CMD_SHARE_NONE: 'No commands',
                CMD_SHARE_NONEW: 'No new commands âœ“',
                ...
                switch (Game.locale_lang.substring(0, 2)) {
                    case 'nl':
                        translate = {
                            ...
                            MY_TEAMS_CONTRIBUTE: 'GeÃ¯ndexeerd rapporten en geÃ¼ploade bevelen worden gedeeld met het team als je de \'Bijdragen\' checkbox aanvinkt.',
                            TEAM_NAME: 'Team naam',
                            TEAM_ROLE: 'Jouw rol',
                ...
                try {
                    ...
                    <div class="button_new disabled" id="gd_cmd_vrvw_share" name="Share with team" style="float: right; margin: 2px; " rel="#gpwnd_1000">
                        ...
                        <div class="caption js-caption">
                            ...
                            <div class="gd_text" style="margin-left: 17px;">Loading â™»</div>
                    ...
                    if (num_updates <= 0) {
                        ...
                        uploadAllCommands(upload_commands, missing_tracked_commands, missing_tracked_command_ids);
            ...
            function uploadPlannedCommands(planned_commands) {
                ...
                uploading_plan = true;
                updateOpsSyncButton('Uploading â™»', true, '', null, '#gd_cmd_plan_share');
                ...
                getAccessToken().then(access_token => {
                    if (access_token === false) {
                        ...
                        $('.rh' + reportHash).each(function () {
                            ...
                            $(this).find('.middle').get(0).innerText = translate.ADDED + ' âœ“';
                            $(this).off("click");
                        });
                        $.ajax({
                                    ...
                                    getAccessToken(true).then(access_token => {
                                        if (access_token === false) {
                                            ...
                                            $('.rh' + reportHash).each(function () {
                                                ...
                                                $(this).find('.middle').get(0).innerText = translate.ERROR + ' âœ—';
                                                $(this).off("click");
                                            });
                                        } else {
                ...
                getAccessToken().then(access_token => {
                    if (access_token === false) {
                        ...
                        if (gd_settings.inbox === true) {
                            ...
                            btn.innerText = translate.ADDED + ' âœ“';
                        }
                        $.ajax({
                            url: backend_url + "/indexer/v2/indexreport",
                            data: data,
                                ...
                                if (error.responseJSON.error_code
                                    ...
                                    getAccessToken(true).then(access_token => {
                                        if (access_token === false) {
                                            ...
                                            btn.innerText = translate.ERROR + ' âœ—';
                                        } else {
                                            // try again with new token
                    ...
                    if (reportElement != null) {
                        ...
                        if (footerText.indexOf('gd_index_rep_') < 0
                            ...
                            if (reportFound) {
                                ...
                                txtSpan.innerText = translate.ADDED + ' âœ“';
                            } else {
                        ...
                        for (var i = 0; i < reportsInView.length; i++) {
                            ...
                            if (reportId && !$('#gd_index_f_' + reportId).get(0)) {
                                ...
                                if (exists === true) {
                                    ...
                                    $('#gd_index_f_txt_' + reportId).get(0).innerText = translate.ADDED + ' âœ“';
                                } else {
                ...
                try {
                    if (!$("#gd_indexer").get(0)) {
                        ...
                        settingsHtml += (count > 0 ? '<p>' + translate.COUNT_1 + count + translate.COUNT_2 + '.</p>' : '') +
                            '<p id="gd_s_saved" style="display: none; position: absolute; left: 10px; margin: 0; color: green;"><strong>' + translate.SAVED + ' âœ“</strong></p> ' +
                            '<br/>\n';
